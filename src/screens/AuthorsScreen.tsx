...
export type AuthorsScreenProps = NativeStackScreenProps<
  RootStackParamList,
  "Authors"
>;
...
export const AuthorsScreen: React.FC<AuthorsScreenProps> = ({ navigation }) => {
  ...
  const rambamOnlySelected = useMemo(() => {
    if (selected.length !== 1) {
      return false;
    }

    const picked = authors.find((author) => author._id === selected[0]);
    if (!picked) {
      return false;
    }

    const normalizedName = picked.name.replace(/["'\s״]/g, "");
    return normalizedName === "רמבם" || normalizedName === "הרמבם";
  }, [authors, selected]);

  const handleContinue = () => {
    setFavoriteAuthors(selected);

    if (notificationTime === "11:07" && rambamOnlySelected) {
    ...
    <ScreenContainer>
      <GlassCard>
        ...
        <Text style={styles.subtitle}>
          אפשר לבחור יותר ממחבר אחד, ואפשר גם לערבב עבור המלצה מהירה.
        </Text>
        <PrimaryButton
          label="בחר את כולם"
          ...
          <View style={styles.chipsContainer}>
            {authors.map((author) => (
              <AuthorChip
                key={author._id}
                name={author.name}
                selected={selected.includes(author._id)}
                onToggle={() => toggleAuthor(author._id)}
              />
            ))}
          </View>
        )}
